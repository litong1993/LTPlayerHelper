...
class AudioHelper: NSObject {
    ...
    /// 用来保存进度回调闭包,progress:当前进度，current：当前播放时间／秒，total：当前歌曲时长／秒
    private var progressBlock:((_ progress:Float,_ current:Double,_ total:Double)->Swift.Void)?
    
    /// 设置进度回调闭包
    ///
    /// - parameter progress: 回调闭包
    func progressBlock(progress:((_ progress:Float,_ current:Double,_ total:Double)->Swift.Void)?){
        ...
        return queuePlayer.addPeriodicTimeObserver(forInterval: CMTimeMake(Int64(1.0), Int32(1.0)), queue: DispatchQueue.main) { [unowned self](time) in
            ...
            if let total = self.queuePlayer.currentItem?.duration.seconds{
                if total.isNaN == false{
                    ...
                    self.progressBlock?(self.progress, current, total)
                }
            }
        }
    }
